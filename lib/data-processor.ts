...
export function filterData(
  ...
  // Check if this is Geography Mode with BY GEOGRAPHY segment type
  // In this case, the geography filter contains segment names (Metro, Non-metro, Rural)
  const isGeographyModeWithByGeography = filters.viewMode === 'geography-mode' &&
    filters.segmentType === 'BY GEOGRAPHY'

  const filtered = data.filter((record) => {
    // 1. Geography filter - must match first
    // SPECIAL CASE: For BY GEOGRAPHY in geography mode, filter by segment name instead
    let geoMatch: boolean
    if (isGeographyModeWithByGeography) {
      // Filter by segment name (Metro, Non-metro, Rural)
      geoMatch = filters.geographies.length === 0 ||
        filters.geographies.includes(record.segment)
    } else {
      geoMatch = filters.geographies.length === 0 ||
...
export function prepareGroupedBarData(
  ...
  return years.map(year => {
    ...
    records.forEach(record => {
      ...
      // Check if this is Geography Mode with BY GEOGRAPHY segment type
      // In this case, Metro/Non-metro/Rural are stored as segments, not geographies
      const isGeographyModeWithByGeography = viewMode === 'geography-mode' &&
        filters.segmentType === 'BY GEOGRAPHY'
      ...
      records.forEach(record => {
        ...
        } else if (isGeographyModeWithByGeography) {
          // SPECIAL CASE: For BY GEOGRAPHY segment type, use segment as key
          // because Metro/Non-metro/Rural are stored as segments
          key = record.segment
        } else if (viewMode === 'geography-mode') {
          key = record.geography
  ...
  return years.map(year => {
    ...
    records.forEach(record => {
      ...
      if (viewMode === 'segment-mode') {
        ...
        // Check if this is BY GEOGRAPHY segment type
        // In that case, Metro/Non-metro/Rural are stored as segments
        if (filters.segmentType === 'BY GEOGRAPHY') {
          key = record.segment
        } else {
...
export function prepareWaterfallData(
  ...
  // Special case: For BY GEOGRAPHY segment type in geography mode, use 'segment' as the key
  const isGeographyModeWithByGeography = filters.viewMode === 'geography-mode' &&
    filters.segmentType === 'BY GEOGRAPHY'
  const groupKey = filters.viewMode === 'segment-mode' || isGeographyModeWithByGeography
    ? 'segment'
    : 'geography'
...
export function prepareMultiLevelChartData(
  ...
  return years.map(year => {
    ...
    // Special case: For BY GEOGRAPHY segment type in geography mode, use segment as key
    const isGeographyModeWithByGeography = viewMode === 'geography-mode' &&
      filters.segmentType === 'BY GEOGRAPHY'

    records.forEach(record => {
      // Determine display key based on view mode
      // Use segment for segment-mode OR when BY GEOGRAPHY in geography mode
      const displayKey = (viewMode === 'segment-mode' || isGeographyModeWithByGeography)
        ? record.segment
        : record.geography
...
export function prepareIntelligentMultiLevelData(
  ...
  records.forEach(record => {
    // Use segment as key for segment-mode OR when BY GEOGRAPHY in geography mode
    const key = (viewMode === 'segment-mode' || isGeographyModeWithByGeography)
      ? record.segment
      : record.geography

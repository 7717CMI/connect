...
export async function processJsonDataAsync(
  ...
  try {
    ...
    // We need to process volume data with volumeData as the primary source
    if (volumeData) {
      console.log('Processing volume data separately...')
      for (const segmentType of segmentTypes) {
        // Call processSegmentTypeAsync with volumeData as the value source
        // Pass null for volumeData param since we're using volumeData as the primary value source
        const { records: volumeRecs } = await processSegmentTypeAsync(
          structureData,
          volumeData,  // Use volume data as the primary value source
          null,        // No separate volume data needed
          segmentType,
          geographies,
          allYears,
          segmentTypeIndex
      ...
      console.log(`Volume records processed: ${volumeRecords.length}`)
    }

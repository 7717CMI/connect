...
export function GroupedBarChart({ title, height = 400 }: GroupedBarChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    // SPECIAL CASE: Geography Mode with BY GEOGRAPHY segment type
    // In this case, Metro/Non-metro/Rural are stored as SEGMENTS, not geographies
    // We need to treat them as geographical breakdowns
    ...
    if (isStacked) {
      ...
      if (effectiveAggregationLevel === 1) {
        ...
        // SPECIAL CASE: Geography Mode with BY GEOGRAPHY segment type
        // Use segments (Metro, Non-metro, Rural) as the series since they represent geographical breakdowns
        series = getUniqueSegments(filtered)
        console.log('📊 Geography Mode with BY GEOGRAPHY - using segments as series:', series)
      } else {

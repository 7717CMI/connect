...
export function EnhancedFilterPanel() {
  ...
  // When switching data type, preserve geography selection
  // Volume and value data share the same segment types, so no need to reset
  useEffect(() => {
    // No special handling needed - both value and volume use same segment types
    // Just ensure the segment type is valid for the current data
    if (data?.dimensions?.segments && !data.dimensions.segments[selectedSegmentType]) {
      const firstSegmentType = Object.keys(data.dimensions.segments)[0]
      if (firstSegmentType) {
        setSelectedSegmentType(firstSegmentType)
        updateFilters({
  ...
  }, [filters.dataType, data])

  // When Geography Mode is selected, automatically switch to BY GEOGRAPHY segment type
  // because Metro/Non-metro/Rural data only exists in that segment type
  useEffect(() => {
    if (filters.viewMode === 'geography-mode' && filters.segmentType !== 'BY GEOGRAPHY') {
      // Check if BY GEOGRAPHY segment type exists
      if (data?.dimensions?.segments?.['BY GEOGRAPHY']) {
        console.log('🌍 Geography Mode: Auto-switching to BY GEOGRAPHY segment type')
        setSelectedSegmentType('BY GEOGRAPHY')
        setSelectedSegments([])
        setCascadePath([])
        setCurrentSegmentSelection('')
        updateFilters({
          segmentType: 'BY GEOGRAPHY',
          geographies: [], // Clear geographies since they will change to Metro/Non-metro/Rural
          segments: [] // Clear segments
        })
      }
    }
  }, [filters.viewMode, data])
  ...
  // Get all segment types - both value and volume data share the same segment structure

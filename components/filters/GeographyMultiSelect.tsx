...
export function GeographyMultiSelect() {
  ...
  // SPECIAL CASE: When segment type is BY GEOGRAPHY in Geography Mode,
  // show Metro/Non-metro/Rural from the segment dimension instead of geographies
  const geographyOptions = useMemo(() => {
    if (!data || !data.dimensions?.geographies) return []

    // Check if we should show BY GEOGRAPHY segments instead
    const isGeographyModeWithByGeography = filters.viewMode === 'geography-mode' &&
      filters.segmentType === 'BY GEOGRAPHY'

    let allGeographies: string[] = []

    if (isGeographyModeWithByGeography) {
      // Get segments from BY GEOGRAPHY dimension (Metro, Non-metro, Rural)
      const byGeoSegment = data.dimensions.segments?.['BY GEOGRAPHY']
      if (byGeoSegment?.items) {
        allGeographies = byGeoSegment.items
      } else if (byGeoSegment?.hierarchy) {
        // Get all keys from hierarchy
        allGeographies = Object.keys(byGeoSegment.hierarchy)
      }
    } else {
  ...
  }, [data, searchTerm, filters.viewMode, filters.segmentType])
  ...
  const handleSelectAll = () => {
    if (!data) return
    // Include all available geography options (could be Metro/Non-metro/Rural if BY GEOGRAPHY)
    updateFilters({
